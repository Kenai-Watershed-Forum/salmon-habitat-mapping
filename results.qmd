...
### how many unique sampling events (unique date/site) did we have 2021-2023?
...
### how many unique event days did we have 2021-2023?
...
#### table of mu,ber of site visits
...
```{r echo = F}
#| label: tbl-sites
#| tbl-cap: "Site visit count 2021 - 2023"
...
#z <- left_join(sample_events,n_sites) %>%
#  select(site,latitude,longitude,site_visits) %>%
#  distinct()
#write.csv(z,"output/site_visits.csv", row.names = F)
...
Fish capture data was evaluated against the 2021 and 2022 Anadromous Waters Catalog to identify new stream segment nominations. Complete submission materials may be accessed in the project [Google Drive folder (link)](https://drive.google.com/drive/folders/1UmQGruxbmRO-ICCd0xbGi5rPp1ZTFCX2?usp=drive_link). Nominations were sent to the ADF&G Habitat Division in Anchorage in the fall of each year.

::: {.callout-note appearance="simple"}

## Total Anadromous Waters Nominations

As of November 2023, a total length of **17.5 miles** of stream segments and **1152 lake acres** were included in the nominations.

:::

Table @tbl-awc-noms below breaks down each stream segment nomination by length.
...
### nomination records
noms_tbl <- read_excel("input/2021_2023_stream_lengths.xlsx") %>%
  clean_names() %>%
...
# table
noms_tbl %>%
  kbl(caption = "AWC Nominations 2021 - 2023", digits = 2) %>%
  kable_paper(full_width = F, html_font = "Cambria")
...
#| fig-cap: "Nominated stream segments highlighted in orange, green, and yellow. Prexisting documented andromous waters prior to this project colored in pink."
knitr::include_graphics('images/2021_2023_awc_noms.jpg')
...
Volunteer recruitment efforts resulted in a steadily increasing level of participation from the general public. A total of 310 volunteer hours were documented throughout the project, with an initial higher outlay in 2021 to initiate training.
...
# plot
vol_hrs %>%
  ggplot(aes(year,hrs,fill = training_vs_fieldwork)) +
  geom_bar(stat = "identity", position = "dodge") +
  xlab("") +
  ylab("Volunteer Hours") +
  theme(legend.title=element_blank()) +
  ggtitle("Central Kenai Peninsula Salmon Habitat Mapping\nVolunteer Hours 2021-2023")

```

The level of volunteer participation, along with experience gained, closely tracked the quantity of nominations submitted each year, with a steadily rising number of nominations from 2021 to 2023.
