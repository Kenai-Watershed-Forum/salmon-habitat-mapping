...
## -----------------------------------------------------------
## 2) Build a lookup that maps *either* common OR scientific
##    name to the canonical common_name used in the dropdown
## -----------------------------------------------------------

species_lookup <- species_map_raw %>%
  mutate(canonical = common_name) %>%              # preserve canonical label
  pivot_longer(
    cols      = c(common_name, scientific_name),  # all possible name forms
    names_to  = "col_type",
    values_to = "name"
  ) %>%
  filter(!is.na(name), name != "") %>%
  mutate(
    species_key = str_to_lower(str_squish(name))   # normalized key for matching
  ) %>%
  select(species_key, canonical) %>%
  distinct()

## -----------------------------------------------------------
## 3) Apply mapping to fish_dat$species
##    - Any value matching common OR scientific gets replaced
##      with canonical common_name
## -----------------------------------------------------------

z <- dat_field_maps %>%
  mutate(
    species_raw = species,
    species_key = str_to_lower(str_squish(as.character(species)))
  ) %>%
  left_join(species_lookup, by = "species_key") %>%
  mutate(
    species = if_else(!is.na(canonical), canonical, species_raw)
  ) %>%
  select(-species_raw, -species_key, -canonical)

# common names now present for all fish species observations

# working here
# this join is supposed to chnage all species names over toc ommon name, but it is not working yet
