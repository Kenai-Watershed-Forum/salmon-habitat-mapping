...
```{r echo = F}
# Note
# In 2025 KWF transitioned from using paper field forms entered in a google sheet to using ESRI Field Maps to collect field data. The code chunks below treat these data read-in processes separately. For 2025, the ERSI data is being formatted to match the pre-existing data structure that was in place during the google sheets read-in process. Fish capture data vs. sampling effort are organized separately.
...
# the data pipline of Field Maps --> ArcGIS Online --> download csv and open in Excel removes the 8 hour UTC time zoen offset. Thus we need to revert all date/times back to local Alaska time
...
## Create a single 'earliest_time' and 'latest_time' column across all datetime columns
dat_field_maps <- dat_field_maps |>
  rowwise() |>
  mutate(site_arrive_time = min(c_across(all_of(datetime_cols)), na.rm = TRUE),
         site_depart_time = max(c_across(all_of(datetime_cols)), na.rm = TRUE)) |>
  ungroup()
