...
dat_field_maps <- read.csv("input/field_maps/kwf_stream_surveys_20251129.csv")
...
dat_field_maps <- dat_field_maps %>%
  ...
  mutate(disposition = case_when(
      # 1. Euthanized all blackfish
      species == "Dallia pectoralis" ~ 
        "Sacrificed and Discarded at Collection Site",
      
      # 2. Species column is missing or recorded as "None"
      is.na(species) ~ NA_character_,
      str_to_lower(species) == "none" ~ NA_character_,
      
      # 3. All other cases
      TRUE ~ "ID'd and released"))%>%              
...
fish_all <- bind_rows(dat_field_common, fish_dat_common) |>
  # modify site name column such that site name is coordinates if o given site name is recorded
  mutate(site = case_when(
    is.na(site) | site == "" ~ paste0(latitude, ",", longitude),
      TRUE ~ site
    
  ))
  
# dataframe contains a single row for each observation of fish species/site/date
```
...
# 12/1/2025 working here to address logic for how to group and count


n_sites_coords <- fish_all %>%
  distinct(site_depart_date,latitude,longitude) %>%
  group_by(latitude,longitude) %>%
  tally()
  group_by(site_depart_date,latitude, longitude) %>%
  tally() %>%
...
-   Table @tbl-sites-visits summarises total sampling events by individual site. We typically sampled a site only one time each.

-   We have captured `r fish_ct` fish comprised of `r spp_ct` unique species. Table @tbl-ct-spp-total summarizes current fish capture count by species.

-   Table @tbl-site-spp-by-site summarizes total fish capture for each species by site.

```{r echo = F}
#| label: tbl-sites-visits
#| tbl-cap: "Site visit count 2021 - 2025"

tbl_sites <- n_sites_coords

if (knitr::is_html_output()) {
  tbl_sites %>%
    kbl(caption = "Total site visits") %>%
    kableExtra::kable_paper(full_width = FALSE, html_font = "Cambria") %>%
    kableExtra::scroll_box(height = "500px", width = "500px")
} else {
  tbl_sites %>%
    knitr::kable(caption = "Total site visits")
}

# export csv and embed for download
dir <- "output/site_visits_coords.csv"
write.csv(n_sites_coords,dir, row.names = F)
embed_file(dir, text = "Download Total Site Visits csv file")
...
if (knitr::is_html_output()) {
  spp_ct_n %>%
    ...
    kableExtra::kable_paper(full_width = FALSE, html_font = "Cambria")  %>%
    kableExtra::scroll_box(height = "600px", width = "500px")
} else {
  spp_ct_n %>%
    knitr::kable(caption = "Total fish capture count by species")
}

# export csv and embed for download
dir <- "output/spp_ct_n.csv"
write.csv(spp_ct_n,dir, row.names = F)
embed_file(dir, text = "Download total fish capture count by species csv file")
...
if (knitr::is_html_output()) {
  ...
  site_spp_ct %>%
    knitr::kable(caption = "Total fish capture count by species at each site.")
}

# export csv and embed for download
dir <- "output/site_spp_ct.csv"
write.csv(site_spp_ct,dir, row.names = F)
embed_file(dir, text = "Download fish species count by site csv file")
...
Tables @tbl-awc-stream-noms and @tbl-awc-lake-noms below breaks down each stream segment nomination by length and lake by area.

```{r echo = F}
#| label: tbl-awc-stream-noms
...
if (knitr::is_html_output()) {
  ...
  noms_tbl %>%
    knitr::kable(caption = "AWC Stream Nominations 2021 - 2025", digits = 2)
}
...
if (knitr::is_html_output()) {
  ...
  noms_tbl_lakes %>%
    knitr::kable(caption = "AWC Lake Nominations 2021 - 2025", digits = 2)
}
...
Or, see the presentation from the 2024 American Fisheries Society National Meeting below on Kenai Watershed Forum's YouTube channel.

[![Kenai Watershed Forum Presentation at the 2024 National Meeting of the American Fisheries Society](https://img.youtube.com/vi/u62Md_aYzPk/hqdefault.jpg)](https://www.youtube.com/watch?v=u62Md_aYzPk)
...
# format ESRI Field Maps effort database to be compatibe with google sheets data (location, departure date, gear type)
dat_field_maps <- dat_field_maps |>
  filter(!is.na(species)) %>%
  # some sites have electrofishing only, some sites minnow traps only
  # in 2025, no sites have both. Field Maps form should be used to record seperate events for MT vs e-fishing; data for the two different capture methods should not be regarded as part of the same event
  mutate(gear_type = case_when(
    !is.na(trap_deployment_time) ~ "Minnow Trap",
    !is.na(electrofishing_start_time) ~ "Backpack Electrofisher"# 
  )) |>
  select(latitude, longitude, site_depart_date, gear_type) |>
  # make site name lat and long
  mutate(site = paste0(latitude,",",longitude)) %>%
  select(-latitude,-longitude) %>%
  transform(site_depart_date = as.POSIXct(site_depart_date))
  

# join google sheets and ESRI data into same dataframe

sample_effort <- bind_rows(dat_field_maps,sample_effort)

  # 11/29/2025 - worked on how to address instances where multiple gear methods existin same event space. None f these events in 2025 captured fish, so a moot point. Note for future years reporting


# join sample effort data
fish_dat <- left_join(fish_all,sample_effort, by = c("site","site_depart_date"))
...
arp <- fish_dat %>%
  ...
  select(-site_arrive_time, -site_depart_time, -count, -project_org,
         -site_arrive_date) %>%
  ...
  # order columns to match ARP column structure
  select(
  ...
  # remove instances where no fish were collected
  filter(!is.na(Species))
